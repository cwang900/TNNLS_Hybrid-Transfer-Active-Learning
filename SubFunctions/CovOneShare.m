...
function K = CovOneShare(theta,X1,X2,SeqX1,SeqX2,NStream)
    D = size(theta,2);
    if D == 1
...
%                                                 (lambda_square_i0j0);
% lambda_square_i0j0 = sqrt(lambda_ij(1,[1 3])'+lambda_ij(2,[1 3]));
% scale_const_i0j0 = sqrt(2)*((lambda_ij(1,[1 3])'.*lambda_ij(2,[1 3]))).^(1/4)./...
%                                                 (lambda_square_i0j0);
