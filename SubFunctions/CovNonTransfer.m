...
function K = CovNonTransfer(theta,X1,X2,SeqX1,SeqX2,NStream)

K = CovCal(theta,X1,X2,SeqX1,SeqX2,NStream);

end


function K = CovCal(theta,X1,X2,SeqX1,SeqX2,NStream)
    D = size(theta,2);
    if D == 1
...
% lambda_square_i0j0 = ((lambda_ij(1,[1 3])').^2+lambda_ij(2,[1 3]).^2);
% scale_const_i0j0 = sqrt(2)*((lambda_ij(1,[1 3])'.*lambda_ij(2,[1 3]))).^(1/2)./...
%                                                 sqrt(lambda_square_i0j0);

lambda_square_i0j0 = sqrt(lambda_ij(1,1:2:2*NStream)'+lambda_ij(2,1:2:2*NStream));
scale_const_i0j0 = sqrt(2)*((lambda_ij(1,1:2:2*NStream)'.*lambda_ij(2,1:2:2*NStream))).^(1/4)./...
                                                (lambda_square_i0j0);
...
if rho_ij(1,2) ~= 0
    lambda_square_iijj = sqrt(lambda_ij(1,2:2:2*NStream).^2'+lambda_ij(2,2:2:2*NStream).^2);

    scale_const_iijj = sqrt(2*(lambda_ij(1,2:2:2*NStream)'.*lambda_ij(2,2:2:2*NStream)))./...
        lambda_square_iijj;
...
rho_square_i0j0 = rho_ij(1,1:2:2*NStream)'.*rho_ij(2,1:2:2*NStream);
rho_square_i0j0 = repelem(rho_square_i0j0,SeqX1,SeqX2);

if rho_ij(1,2) ~= 0
    rho_square_iijj = rho_ij(1,2:2:2*NStream)'.*rho_ij(2,2:2:2*NStream);
