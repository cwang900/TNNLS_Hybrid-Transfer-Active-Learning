...
xS = pd.read_excel('.../XSource8Streams.xlsx', header=None)
yS = pd.read_excel('.../ySource8Streams.xlsx', header=None)
xT = pd.read_excel('.../XTarget8Streams.xlsx', header=None)
yT = pd.read_excel('.../yTarget8Streams.xlsx', header=None)
xP = pd.read_excel('.../XPool8Streams.xlsx', header=None)
yP = pd.read_excel('.../yPool8Streams.xlsx', header=None)
xR = pd.read_excel('.../XRand8Streams.xlsx', header=None)
yR = pd.read_excel('.../yRand8Streams.xlsx', header=None)
yTrue = pd.read_excel('.../yTrue8Streams.xlsx', header=None)

m=20
class CustomModel(nn.Module):
    def __init__(self):
        super(CustomModel, self).__init__()
        self.layer1 = nn.Linear(1, m)  # 第一层
        ...
        self.layer5 = nn.Linear(m, 1)  # 输出层3
        self.layer6 = nn.Linear(m, 1)  # 输出层4
        self.layer7 = nn.Linear(m, 1)  # 输出层5
        self.layer8 = nn.Linear(m, 1)  # 输出层6
        self.layer9 = nn.Linear(m, 1)  # 输出层7
        self.layer10 = nn.Linear(m, 1)  # 输出层8
    ...
    def forward(self, x):
        ...
        x3 = self.layer5(x)   # 输出3
        x4 = self.layer6(x)  # 输出4
        x5 = self.layer7(x)   # 输出5
        x6 = self.layer8(x)  # 输出6
        x7 = self.layer9(x)   # 输出7
        x8 = self.layer10(x)  # 输出8
        x = torch.cat((x1, x2, x3, x4, x5, x6, x7, x8), dim=1)  # 使用torch.cat直接将x1和x2合并成一个2列的张量
        return x
...
NTOnline = 200 # Number of training steps during online learning
NStream = 8 # Number of Streams
...
for l in range(NRep):
    ...
    x_train = torch.tensor(xS.values).float()[:,l].unsqueeze(1) # load data
    ...
    for epoch in range(NSPretrain):
        ...
        loss3 = ((y_pred[:, 2] - y_train[0:, 2]) ** 2).mean()
        loss4 = ((y_pred[:, 3] - y_train[0:, 3]) ** 2).mean()
        loss5 = ((y_pred[:, 4] - y_train[0:, 4]) ** 2).mean()
        loss6 = ((y_pred[:, 5] - y_train[0:, 5]) ** 2).mean()
        loss7 = ((y_pred[:, 6] - y_train[0:, 6]) ** 2).mean()
        loss8 = ((y_pred[:, 7] - y_train[0:, 7]) ** 2).mean()
    ...
    x_train = torch.tensor(xT.values).float()[:,l].unsqueeze(1)
    y_train = torch.tensor(yT.values).float()[:,NStream*l:NStream*l+NStream]
    x_pool = torch.tensor(xP.values).float()[:,l].unsqueeze(1)
    ...
    xx = torch.tensor(xP.values).float()[::50,l].unsqueeze(1)
    ...
    pool_rmse3 = ((yp_pred[:, 2] - y_true[:, 2]) ** 2).mean()
    pool_rmse4 = ((yp_pred[:, 3] - y_true[:, 3]) ** 2).mean()
    pool_rmse5 = ((yp_pred[:, 4] - y_true[:, 4]) ** 2).mean()
    pool_rmse6 = ((yp_pred[:, 5] - y_true[:, 5]) ** 2).mean()
    pool_rmse7 = ((yp_pred[:, 6] - y_true[:, 6]) ** 2).mean()
    pool_rmse8 = ((yp_pred[:, 7] - y_true[:, 7]) ** 2).mean()
    pool_rmse = (pool_rmse1 + pool_rmse2 + pool_rmse3 + pool_rmse4 + pool_rmse5 + pool_rmse6 + pool_rmse7 + pool_rmse8) / 8
    pool_rmse = pool_rmse.sqrt().item()

    pool_mape1 = ((yp_pred[:, 0] - y_true[:, 0]) / y_true[:, 0]).abs()
    pool_mape2 = ((yp_pred[:, 1] - y_true[:, 1]) / y_true[:, 1]).abs()
    pool_mape3 = ((yp_pred[:, 2] - y_true[:, 2]) / y_true[:, 2]).abs()
    pool_mape4 = ((yp_pred[:, 3] - y_true[:, 3]) / y_true[:, 3]).abs()
    pool_mape5 = ((yp_pred[:, 4] - y_true[:, 4]) / y_true[:, 4]).abs()
    pool_mape6 = ((yp_pred[:, 5] - y_true[:, 5]) / y_true[:, 5]).abs()
    pool_mape7 = ((yp_pred[:, 6] - y_true[:, 6]) / y_true[:, 6]).abs()
    pool_mape8 = ((yp_pred[:, 7] - y_true[:, 7]) / y_true[:, 7]).abs()
    pool_mape = (pool_mape1 + pool_mape2 + pool_mape3 + pool_mape4 + pool_mape5 + pool_mape6 + pool_mape7 + pool_mape8).sum().item() / 8 / pool_mape1.shape[0] * 100
    ...
    # Online learning
    x_random = torch.tensor(xR.values).float()[:, l].unsqueeze(1) # load random data
...
if SelectionMethod==1:
    np.savetxt('Random2D8S_RMSE.csv', wl_loss, delimiter=',', header=','.join([f' {i+1}' for i in range(NRep)]), comments='')
    np.savetxt('Random2D8S_MAPE.csv', wl_mape, delimiter=',', header=','.join([f' {i + 1}' for i in range(NRep)]),
               comments='')
    np.savetxt('Random2D8S_Time.csv', ExeTime, delimiter=',',
               header=','.join([f' {i + 1}' for i in range(NRep)]), comments='')
else:
    np.savetxt('Active2D8S_RMSE.csv', wl_loss, delimiter=',', header=','.join([f' {i+1}' for i in range(NRep)]), comments='')
    np.savetxt('Active2D8S_MAPE.csv', wl_mape, delimiter=',', header=','.join([f' {i + 1}' for i in range(NRep)]),
               comments='')
    np.savetxt('Active2D8S_Time.csv', ExeTime, delimiter=',',
               header=','.join([f' {i + 1}' for i in range(NRep)]), comments='')
