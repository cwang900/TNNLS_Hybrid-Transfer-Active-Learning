...
xS = pd.read_excel('.../XSource8Domain.xls', header=None)
yS = pd.read_excel('.../ySource8Domain.xls', header=None)
xT = pd.read_excel('.../XTarget8Domain.xls', header=None)
yT = pd.read_excel('.../yTarget8Domain.xls', header=None)
xP = pd.read_excel('.../XPool8Domain.xls', header=None)
yP = pd.read_excel('.../yPool8Domain.xls', header=None)
xR = pd.read_excel('.../XRand8Domain.xls', header=None)
yR = pd.read_excel('.../yRand8Domain.xls', header=None)
yTrue = pd.read_excel('.../yTrue8Domain.xls', header=None)
...
NSPretrain = 2000 # Number of training steps with source data
...
for l in range(NRep):
    ...
    y_train = torch.tensor(yS.values).float()[:,2*l:2*l+2]
    ...
    y_train = torch.tensor(yT.values).float()[:,2*l:2*l+2]
    x_pool = torch.tensor(xP.values).float()[:,l].unsqueeze(1)
    y_pool = torch.tensor(yP.values).float()[:,2*l:2*l+2]
    y_true = torch.tensor(yTrue.values).float()[:,2*l:2*l+2]
    ...
    for epoch in range(NTOffline):
        ...
        loss1 = ((y_pred[:, 0] - y_train[:, 0]) ** 2).mean()
        loss2 = ((y_pred[:, 1] - y_train[:, 1]) ** 2).mean()
        loss = (loss1 + loss2) / 2
        loss.backward()
        opt.step()
        opt.zero_grad()
    ...
    y_random = torch.tensor(yR.values).float()[:, 2 * l:2 * l + 2]
    ...
    for epoch in range(NRound-1):
        ...
        x_train = torch.cat((x_train, x_selected), dim=0)
        y_train = torch.cat((y_train, y_selected), dim=0)
...
if SelectionMethod==1:
    np.savetxt('RandomPoly_RMSE.csv', wl_loss, delimiter=',', header=','.join([f' {i+1}' for i in range(NRep)]), comments='')
    np.savetxt('RandomPoly_MAPE.csv', wl_mape, delimiter=',', header=','.join([f' {i + 1}' for i in range(NRep)]),
               comments='')
    np.savetxt('RandomPoly_Time.csv', ExeTime, delimiter=',',
               header=','.join([f' {i + 1}' for i in range(NRep)]), comments='')
else:
    np.savetxt('ActivePoly_RMSE.csv', wl_loss, delimiter=',', header=','.join([f' {i+1}' for i in range(NRep)]), comments='')
    np.savetxt('ActivePoly_MAPE.csv', wl_mape, delimiter=',', header=','.join([f' {i + 1}' for i in range(NRep)]),
               comments='')
    np.savetxt('ActivePoly_Time.csv', ExeTime, delimiter=',',
               header=','.join([f' {i + 1}' for i in range(NRep)]), comments='')
