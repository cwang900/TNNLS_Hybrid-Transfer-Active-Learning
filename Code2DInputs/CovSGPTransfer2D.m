...
function K = CovSGPTransfer2D(theta,X1,X2,SeqX1,SeqX2)
    NStream = [length(SeqX1) length(SeqX2)];    
    D = size(X1,2);
    Hyper.Rho = theta(1:sum(NStream),:);
    Hyper.lambda = theta(sum(NStream)+(1:D*sum(NStream)),:);
