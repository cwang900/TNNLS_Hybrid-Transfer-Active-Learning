function X = OPTIMSE2D(XSample0,XStar,Sigma,XInd,theta,NDomain,CovType,lb,ub,NStream)

[NS,ND] = size(XSample0);
opt.algorithm = NLOPT_LD_MMA;%NLOPT_LN_AUGLAG_EQ;
opt.maxeval = 10000;
opt.lower_bounds = lb*ones(1,NS*ND);
opt.upper_bounds = ub*ones(1,NS*ND);
opt.xtol_rel = 1e-4;
...
XX = nlopt_optimize(opt, XSample0(:));
X = XX;
...
XInd0 = XInd;
XStar0 = XStar;
XSample = reshape(XSample,[],D);
XSample = repmat(XSample,NStream(end),1);

yt = zeros(size(XSample,1),1);
alpha = zeros(size(XInd,1),1);
% [~, SigmaT] = PostOnline(theta,yt,XSample,Mu,Sigma,XInd,NDomain,CovType);
[~, SigmaT] = PostInd2D(theta,{yt},{XSample},{XStar},alpha,Sigma,{XInd},NDomain,CovType,NStream);
...
% J = trapz(X1,Y1)+trapz(X2,Y2);
S = diag(SigmaT);
S = reshape(S,[],NStream(end));
NStar = sqrt(size(XStar,1)/2);
J= 0;
for i = 1:NStream(end)
    X1 = XStar((i-1)*NStar^2+1:i*NStar^2,1);
    X1 = reshape(X1,NStar,[]);
    X2 = XStar((i-1)*NStar^2+1:i*NStar^2,2);
    X2 = reshape(X2,NStar,[]);
    YY = reshape(S(:,i),NStar,[]);
    J = J+trapz(X2(:,1),trapz(X1(1,:),YY,2));
end
J = log(J);
% if ~isreal(J)
%     aa;
