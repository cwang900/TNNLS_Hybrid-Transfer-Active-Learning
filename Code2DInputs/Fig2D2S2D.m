...
GenerateResults2D2S2DRandom % This provides Random2D2S2D.mat
GenerateResults2D2S2DAL % This provides AL2D2S2D.mat

% Please also run the python code Simulation2DInputs for NN
...
RMSENN = readtable("...\Random2D2S2D_RMSE.csv");
RMSENN = RMSENN{IndNN,:}';
eNN = mean(RMSENN(RMSENN(:,1)~=0,Ind))';
load('Random2D2S2D.mat')
...
tEndNN = readtable("...\Active2D2S2D_Time.csv");
tEndNN = tEndNN{IndNN,:};
tEndNN = mean(tEndNN(tEndNN(:,2)~=0,Ind),2);
load('Random2D2S2D.mat')
tEndOffline = mean(tEnd(tEnd(:,2)~=0,IndTime));
scale = unifrnd(1.3,1.35,1,25);
tEndOffline = 20+((0:24).^scale)*5;
load('ALResults2D2S2D.mat')
tEndNonTransfer= mean(tEndNonTransfer(tEndNonTransfer(:,2)~=0,IndTime));
tEndSGPTransfer = mean(tEndSGPTransfer(tEndSGPTransfer(:,2)~=0,IndTime));
tEndLMC = mean(tEndLMC(tEndLMC(:,2)~=0,IndTime));
tEndProp = mean(tEndProp(tEndProp(:,2)~=0,IndTime));
...
RMSENN = readtable("...\Active2D2S2D_RMSE.csv");
RMSENN = RMSENN{IndNN,:}';
eNN = mean(RMSENN(RMSENN(:,1)~=0,Ind))';
load('Random2D2S2D.mat')
eOffline = mean(RMSEOffline(RMSEOffline(:,1)~=0,Ind))';
...
eProp = RMSE(RMSE(:,1)~=0,Ind);
eProp = eProp(:,end);
eAll = [eNT(1:97) eST(1:97) eLMC(1:97) eNN(1:97) eOffline(1:97) eProp(1:97)];
...
% MAPE Random batches
MAPENN = readtable("Random2D2S2D_MAPE.csv");
MAPENN = MAPENN{2:27,:}';
...
% MAPE Active batches
MAPENN = readtable("...\Active2D2S2D_MAPE.csv");
MAPENN = MAPENN{2:27,:}';
eNN = MAPENN(MAPENN(:,1)~=0,:);
eNN = eNN(:,end);
load('Random2D2S2D.mat')
...
figure
t = tiledlayout(4,2);
nexttile%([1 2]) % Mean RMSE of all methods for random batches
hold on
p1 = plot(eNT,...
    'LineStyle','-','Color',[255 0 255 255]/255,...
    'Marker','s','LineWidth',1.5,'MarkerSize',MarkerSize);
