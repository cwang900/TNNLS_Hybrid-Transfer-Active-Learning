...
while iAL<LearningRound %Error > 1e-3
    tstart = tic;
    XTemp = XRand(:,NSample*(iAL-1)+1:NSample*iAL);
    XNew = [XNew XTemp(:)];
...
function [RMSE, MAPE, tEnd] = OfflineLearning(X,y,ffun,Xmin,Xmax,theta0,CovType,LearningRound,NStream)
...
% lN = length(y{end})/2;
% yTemp = y;
% yTemp{end} = reshape(yTemp{end},[],NStream(end));
% XTemp = X;
% XTemp{end} = reshape(XTemp{end},lN,D*NStream(end));
% Ind = randperm(lN,lN);
% yTemp{end} = yTemp{end}(Ind,:);
% yTemp{end} = yTemp{end}(:);
% XTemp{end} = XTemp{end}(Ind,:);
...
for i = 0:LearningRound-1
    ...
    yTemp = y;
    yTemp{end} = reshape(yTemp{end},[],NStream(end));
    yTemp{end}(end-NSample*(LearningRound-1-i)+1:end,:) = [];
    yTemp{end} = yTemp{end}(:);
    XTemp = X;
    XTemp{end} = reshape(XTemp{end},lN,D*NStream(end));
    XTemp{end}(end-NSample*(LearningRound-1-i)+1:end,:) = [];
    XTemp{end} = reshape(XTemp{end},[],2);

    theta = OptJOffline2D(theta0,yTemp,XTemp,[],CovType,NStream);
    [Mu, Sigma] = PostInd2D(theta,yTemp,XTemp,{XStar},[],[],{XTInd},NDomain,CovType,NStream);
    
    fStar = Mu;

    RMSE(i+1) = sum((fStar(:)-ff(:)).^2);
    RMSE(i+1) = sqrt(RMSE(i+1)/(2*NObs^2));

    MAPE(i+1) = sum(abs((fStar(:)-ff(:))./ff(:)));
    MAPE(i+1) = 100*(MAPE(i+1)/(2*NObs^2));
